...
namespace Unity.Networking.Transport
{
    ...
    public unsafe struct DataStreamReader
    {
        ...
        public uint ReadPackedUInt(ref Context ctx/*, NetworkCompressionModel model*/)
        {
            ...
            uint peekMask = 1u; // (1u << NetworkCompressionModel.k_MaxHuffmanSymbolLength) - 1u;
            uint peekBits = (uint)ctx.m_BitBuffer & peekMask;
            ushort huffmanEntry = 1; // model.decodeTable[(int)peekBits];
            ...
            uint offset = 1; // model.bucketOffsets[symbol];
            int bits = 1; // model.bucketSizes[symbol];
